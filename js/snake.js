(function() {
  ...
  let speed=300; // Reduced speed from 200 to 300 for better playability
  ...
  function update(){
    ...
    if(head.x===food.x&&head.y===food.y){
      ...
      if(score%5===0&&speed>100){
        speed-=30; // Adjusted speed increment for smoother difficulty scaling
  ...
  canvas.addEventListener('touchend', function(e) {
    ...
    if (tapLength < 300 && tapLength > 0) {
      ...
      if(vx || vy){
        vx *= 2;
        vy *=2;
        setTimeout(() => { vx /= 2; vy /=2; }, 500); // Temporary speed boost
      }
    }
    lastTap = currentTime;
  }, false);
